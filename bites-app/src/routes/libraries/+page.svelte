...
<div class="overflow-x-auto">
	<div
		class="bg-base-300 content-center container place-self-center mx-auto my-4 mb-20 rounded-box overflow-x-scroll"
	>
			...
			{#each Object.entries(labels_to_functions) as [label, function_member], index}
				{#if index == 0}
					...
					<input
						...
						name="my_tabs_1"
						role="tab"
						class="tab lg:ml-8 xl:ml-48 2xl:ml-64"
						aria-label="{label}"
						checked
					/>
				{:else}
				...
				<div
					role="tabpanel"
					class="tab-content p-10 2xl:w-[100%] xl:w-[90%] lg:w-[66%] md:w-[50%] sm:w-[30%] min-[320px]:w-[10%]"
				>
					<LibraryTable libraryTable="{function_member}" {columns} />
				</div>
			{/each}
		</div>
	</div>
